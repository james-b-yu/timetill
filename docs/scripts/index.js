...
let scrollUpdate = e => {
    ...
    for (let i = 0; i < screens.length; i++) {
        ...
        if (Math.abs(screens[i].getBoundingClientRect().top) < window.innerHeight / 2 && i != 0) {
            // do not highlight top button
...
for (let i = 0; i < pickerButtons.length; i++) {
    ...
    pickerButtons[i].onmouseup = () => {
        ...
        for (let j of pickerButtons[i].parentNode.children) j.removeAttribute("current-item");
